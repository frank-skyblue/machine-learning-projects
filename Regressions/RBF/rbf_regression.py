...
class RBFRegression():
    def __init__(self, centers, widths):
        ...
        """
        assert centers.shape[0] == widths.shape[
            0], f"The number of centers and widths must match. (Centers: {centers.shape[0]}, Widths: {widths.shape[0]})"
        assert centers.shape[
            1] == 2, f"Each center should have two components. (Centers: {centers.shape[1]})"
        assert widths.shape[
            1] == 1, f"Each width should have one component. (Widths: {widths.shape[1]})"
    ...
    def predict(self, X):
        ...
        # Pad 1 at the start of the computed basis
        bias = self.parameters[0]
        N = X.shape[0]

        output = np.tile(np.expand_dims(bias, axis=1), reps=(N, 1))

        for i in range(self.K):
            # assuming that self.parameters has bias as first element, we skip the first element
            parameter_i = self.parameters[i + 1]
            computed_basis_i = self._rbf_2d(X=X, rbf_i=i)
            output += parameter_i * computed_basis_i
    ...
    def fit_with_l2_regularization(self, train_X, train_Y, l2_coef):
        ...
        """
        assert train_X.shape[0] == train_Y.shape[
            0], f"Number of inputs and outputs are different. (train_X: {train_X.shape[0]}, train_Y: {train_Y.shape[0]})"
        assert train_X.shape[
            1] == 2, f"Each input should contain two components. Got: {train_X.shape[1]}"
        assert train_Y.shape[
            1] == 1, f"Each output should contain 1 component. Got: {train_Y.shape[1]}"

        # ====================================================
        # TODO: Implement your solution within the box

        B = np.ones((train_X.shape[0], 1))  # paddings
        for i in range(self.K):
            output_basis_i = self._rbf_2d(train_X, i)
            B = np.concatenate((B, output_basis_i), 1)
        self.parameters = (np.linalg.inv(
            B.T @ B + l2_coef * np.identity(self.K + 1))) @ (B.T @ train_Y)
...
if __name__ == "__main__":
    ...
    print("Correct optimal weights: {}".format(
        np.allclose(model.parameters, optimal_parameters)))
